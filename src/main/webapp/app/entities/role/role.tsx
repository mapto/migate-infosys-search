...
import { searchEntities, getEntities, reset } from './role.reducer';
...
export const Role = () => {
  ...
  const links = useAppSelector(state => state.role.links);
  const entity = useAppSelector(state => state.role.entity);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={roleList ? roleList.length : 0}
          next={handleLoadMore}
            ...
            <Table responsive>
              <thead>
                <tr>
                  ...
                  <th>
                    Sponsor <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
              ...
              <tbody>
                {roleList.map((role, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{role.name}</td>
                    <td>{role.start ? <TextFormat type="date" value={role.start} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                    <td>{role.end ? <TextFormat type="date" value={role.end} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                    <td>{role.sponsor ? <Link to={`/institution/${role.sponsor.id}`}>{role.sponsor.name}</Link> : ''}</td>
                    <td>{role.person ? <Link to={`/person/${role.person.id}`}>{role.person.name}</Link> : ''}</td>
                    <td>{role.product ? <Link to={`/work/${role.product.id}`}>{role.product.name}</Link> : ''}</td>
