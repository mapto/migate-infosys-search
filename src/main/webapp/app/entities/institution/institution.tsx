...
import { searchEntities, getEntities, reset } from './institution.reducer';
...
export const Institution = () => {
  ...
  const links = useAppSelector(state => state.institution.links);
  const entity = useAppSelector(state => state.institution.entity);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={institutionList ? institutionList.length : 0}
          next={handleLoadMore}
              ...
              <tbody>
                {institutionList.map((institution, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{institution.position ? <Link to={`/role/${institution.position.id}`}>{institution.position.name}</Link> : ''}</td>
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button
                          tag={Link}
                          to={`/institution/${institution.id}/delete`}
                          color="danger"
                          size="sm"
