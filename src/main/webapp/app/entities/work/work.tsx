...
import { searchEntities, getEntities, reset } from './work.reducer';
...
export const Work = () => {
  ...
  const totalItems = useAppSelector(state => state.work.totalItems);
  const links = useAppSelector(state => state.work.links);
  const entity = useAppSelector(state => state.work.entity);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={workList ? workList.length : 0}
          next={handleLoadMore}
              ...
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('published')}>
                    Published <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    Collection <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    Responsibility <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {workList.map((work, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{work.collection ? <Link to={`/work-group/${work.collection.id}`}>{work.collection.name}</Link> : ''}</td>
                    <td>{work.responsibility ? <Link to={`/role/${work.responsibility.id}`}>{work.responsibility.name}</Link> : ''}</td>
