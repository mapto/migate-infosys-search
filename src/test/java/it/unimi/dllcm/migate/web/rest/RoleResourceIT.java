...
class RoleResourceIT {
    ...
    @Test
    @Transactional
    void getAllRolesByInstitutionNameIsEqualToSomething() throws Exception {
        Institution institutionName;
        if (TestUtil.findAll(em, Institution.class).isEmpty()) {
            roleRepository.saveAndFlush(role);
            institutionName = InstitutionResourceIT.createEntity(em);
        } else {
            institutionName = TestUtil.findAll(em, Institution.class).get(0);
        }
        em.persist(institutionName);
        em.flush();
        role.addInstitutionName(institutionName);
        roleRepository.saveAndFlush(role);
        Long institutionNameId = institutionName.getId();

        // Get all the roleList where institutionName equals to institutionNameId
        defaultRoleShouldBeFound("institutionNameId.equals=" + institutionNameId);

        // Get all the roleList where institutionName equals to (institutionNameId + 1)
        defaultRoleShouldNotBeFound("institutionNameId.equals=" + (institutionNameId + 1));
    }
    ...
    @Test
    @Transactional
    void getAllRolesByWorkNameIsEqualToSomething() throws Exception {
        Work workName;
        if (TestUtil.findAll(em, Work.class).isEmpty()) {
            roleRepository.saveAndFlush(role);
            workName = WorkResourceIT.createEntity(em);
        } else {
            workName = TestUtil.findAll(em, Work.class).get(0);
        }
        em.persist(workName);
        em.flush();
        role.addWorkName(workName);
        roleRepository.saveAndFlush(role);
        Long workNameId = workName.getId();

        // Get all the roleList where workName equals to workNameId
        defaultRoleShouldBeFound("workNameId.equals=" + workNameId);

        // Get all the roleList where workName equals to (workNameId + 1)
        defaultRoleShouldNotBeFound("workNameId.equals=" + (workNameId + 1));
    }
